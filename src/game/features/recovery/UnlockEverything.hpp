...
namespace YimMenu
    {
    inline void unlock_packed_bools(int from, int to)
        {
        for (int i = from; i <= to; ++i)
            Stats::SetPackedBool(i, true);
        };

    inline void SET_MP_INT_CHARACTER_STAT_BIT(std::string statName, int bit)
        {
        auto val = Stats::GetInt(statName);
        val |= bit;
        Stats::SetInt(statName, val);
        }
    inline void SET_MP_INT_CHARACTER_STAT_BITS(std::string statName, int fromBit, int toBit)
        {
        auto val = Stats::GetInt(statName);

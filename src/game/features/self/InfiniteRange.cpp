        ...
        virtual void OnTick() override
            ...
            if (!HUD::IS_PAUSE_MENU_ACTIVE() && !GUI::IsOpen() && Self::GetPed() && !Self::GetPed().IsDead())
                ...
                if (PAD::IS_DISABLED_CONTROL_PRESSED(0, (int)ControllerInputs::INPUT_ATTACK))
                    {
                    static std::uint32_t lastShotMs = 0;
                    const std::uint32_t nowMs = MISC::GET_GAME_TIMER();
                    Hash weapon_hash = Self::GetPed().GetCurrentWeapon();
                    float interval = WEAPON::GET_WEAPON_TIME_BETWEEN_SHOTS(weapon_hash);
                    std::uint32_t cooldown = static_cast<std::uint32_t>(std::max(30.f, interval * 1000.f));
